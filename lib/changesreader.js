...
class ChangesReader {
    ...
    setDefaults() {
        ...
        this.started = false
        this.wait = false
        this.stopOnEmptyChanges = false // whether to stop polling if we get an
                                        // empty set of changes back
    ...
    // called to start listening to the changes feed. The opts object can
    // contain: - batchSize - the number of records to return per HTTP request
        ...
        const work = async () => {
            do {
                if (!self.paused) {
                    ...
                    // make HTTP request to get up to batchSize changes from
                    // the feed
                    try {
                        ...
                        if (data && data.results && data.results.length > 0) {
                            ...
                            // in 'wait' mode, we need to wait until the user
                            // calls .resume()
                            if (self.wait) {
                                // so call pause() to prevent further API calls
                        ...
                        if (err && err.statusCode && err.statusCode >= 400 && err.statusCode !== 429 && err.statusCode < 500) {
                            ...
                            // don't immediately retry on error - exponential
                            // back-off
                            delay = delay ? Math.min(60000, delay * 2) : 5000 // up
                                                                              // to
                                                                              // and
                                                                              // no
                                                                              // more
    ...
    // called to start listening to the changes feed for a finite number of
    // changes. The opts object can contain: - batchSize - the number of
    // records to return per HTTP request - since - the sequence token to start
