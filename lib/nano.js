...
export default function dbScope(cfg) {
    ...
    function scrubRequest(req, cloned) {
        ...
        if (req.auth) {
            if (!cloned) {
                req.auth = JSON.parse(JSON.stringify(req.auth)) // clone just
                                                                // auth if not
                                                                // already
                                                                // cloned
            }
    ...
    function relax(opts, callback) {
        ...
        // going to use axios, so let's modify the "req" object to suit axios
        req.url = req.uri
        delete req.uri
    ...
    function docModule(dbName) {
        ...
        function headDoc(docName, callback) {
            ...
            if (callback) {
                ...
                // this function doesn't pass on the Promise from relax because
                // it needs to return the headers when resolving the Promise
